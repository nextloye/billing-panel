...
// Init DB
db.serialize(()=> {
  db.run("CREATE TABLE IF NOT EXISTS invoices(id INTEGER PRIMARY KEY, user TEXT, plan TEXT, amount INTEGER, screenshot TEXT, status TEXT)");
});
...
app.get('/invoice/:id',(req,res)=>{
  db.get("SELECT * FROM invoices WHERE id=?",req.params.id,(e,row)=>{
    res.render('invoice',{invoice:row, upi:"8755016597@fam"});
  });
});
